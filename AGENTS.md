...
- **`docs/v3/` es el diseño vigente** (razas, 8 Habilidades, Características, combate sin dados). Todo el diseño nuevo se escribe aquí. **Su árbol dejó de ser el de v2 el 6 de septiembre de 2026**: seis apartados, con **`sistemas/`** (lo que llevan las 132 fichas por igual: la ficha, las Habilidades, las Características, el tipo de daño y los estados) y **`razas/`** (lo que sí es de una raza: sus clases y sus unidades) partidos de lo que era un solo `razas.md`. Antes de escribir, mira en [`docs/v3/README.md`](docs/v3/README.md) §"Cómo está organizado" a qué apartado va lo tuyo — un sistema que valga para las 132 no va en `razas/`.
- **Excepción abierta: las razas.** Se están redefiniendo en `knowledge/v3/races-concept/razas.md`, que es el archivo que se edita; lo publicado en `docs/v3/` queda congelado —lo lee la wiki— y solo se reescribe cuando un bloque del concepto está cerrado. **Ese archivo de trabajo sigue entero y su destino ya son seis**: la tabla de a cuál va cada sección está en [`knowledge/v3/races-concept/README.md`](knowledge/v3/races-concept/README.md), y hay que mirarla antes de volcar nada.
