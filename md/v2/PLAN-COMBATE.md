...
> **ARCHIVADO (20 de agosto de 2026) — no ejecutar.** Este es el plan de implementación del motor de combate de **v2**: tirada de impacto d20 contra CA, dado de daño, natural 20, ventaja/desventaja como 2d20, salvaciones por característica. V3 resuelve el combate **sin ningún dado**, así que nada de este plan sirve tal cual.
>
> Se conserva junto a [`docs/v2/`](../../docs/v2/) como referencia de estructura: el reparto por módulos, el alcance acotado de la ronda y el criterio de "validar la matemática en solitario antes de ampliar" siguen siendo buen método, aunque las reglas que implementan estén muertas.
>
> Las rutas que cita son las de antes del corte: `docs/…` es hoy `docs/v2/…`, y `lib/rules/…` es hoy `lib/v2/rules/…`.
