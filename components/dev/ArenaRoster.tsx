...
//
// Las cinco fichas de battle.md §2 —el héroe y cuatro unidades— con lo único
// que el tablero necesita saber de ellas: su tipo de daño y en qué hexágono
// están. Ni Habilidades, ni Vida, ni carta: eso no existe todavía y no se
// inventa aquí (lib/v3/deployment.ts).
//
...
import { hexOf, type Deployment, type Roster } from "@/lib/v3/deployment";
      ...
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <div>
          <div className="font-semibold text-[var(--wiki-text)]">
            Tu bando · {placed} de {roster.length} desplegadas
          </div>
          <p className="mt-0.5 text-xs text-[var(--wiki-muted)]">
            Elige una ficha y pulsa un hexágono de tu banda. Sin ninguna elegida, el tablero vuelve
            a medir distancias.
          </p>
        </div>
      ...
      <ul className="grid gap-1.5">
        {roster.map((figure) => {
          ...
          return (
            ...
            >
              {/* La ficha entera es el botón de elegir: el objetivo de clic es la
                  fila, no un punto de la fila. Vuelve a pulsarse para soltarla. */}
              <button
              ...
              {/* El tipo de daño se puede cambiar porque es lo único decidido de
                  una ficha, y es lo que hace que colocarla sea una decisión. */}
              ...
              <span className="ml-auto flex items-center gap-2 text-xs text-[var(--wiki-muted)]">
                {round !== null && (
                  <span title="Desde donde está, contra el frente enemigo, con este 👢 Movimiento.">
                ...
                {cell && (
                  <button
                    ...
                    aria-label={`Retirar ${figure.label}`}
