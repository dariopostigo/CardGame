  ...
  return (
    <div className="min-h-screen">
      <header className="sticky top-0 z-30 flex h-14 items-center gap-3 border-b border-[var(--wiki-border)] bg-[var(--wiki-surface)]/95 px-4 backdrop-blur">
        ...
        <Link href="/lab" className="flex items-center gap-2 font-semibold">
          <i className="pi pi-slack text-[var(--wiki-accent)]" />
          <span>
            CardGame <span className="text-[var(--wiki-muted)]">Lab</span>
          </span>
        </Link>
        <div className="ml-auto flex items-center gap-1">
          <SectionLinks current="lab" />
          <ThemeControls />
        </div>
      </header>
      ...
      {open && (
        <div className="fixed inset-0 z-40 lg:hidden">
          ...
          <div className="absolute left-0 top-0 h-full w-72 overflow-y-auto bg-[var(--wiki-surface)] shadow-xl">
            ...
            <LabSidebar onNavigate={() => setOpen(false)} />
          </div>
        </div>
      )}
    </div>
