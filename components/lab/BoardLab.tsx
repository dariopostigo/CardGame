...
// =========================================================================
// Laboratorio de generación de TABLERO — /lab/board
//
// Banco de pruebas de lib/v2/rules/board-gen.ts: cambia la semilla y los
...
// cada loseta por dentro se maqueta en el otro laboratorio (/lab/tiles).
//
// Vive en components/lab/ y no en components/game/ porque es instrumental: el
...
import type { BoardToken, Hex as HexCell } from "@/lib/v2/rules/state";
import { TERRAINS, TERRAIN_IDS, type TerrainId } from "@/lib/v2/rules/terrain";
import { TILES_BY_ID } from "@/lib/v2/rules/tile-library";
...
export default function BoardLab() {
  ...
  // añade cuántos tableros correr: no es un panel aparte (lib/lab-registry.ts, nota
  ...
  return (
    <div>
      ...
      <p className="mb-5 max-w-3xl text-sm text-[var(--wiki-muted)]">
        ...
        <Link href="/lab/tiles" className="text-[var(--wiki-accent)] hover:underline">
          losetas
        </Link>{" "}
      ...
      <div className="mb-4 flex flex-wrap gap-x-6 gap-y-1 text-sm text-[var(--wiki-text)]">
        ...
        <span title="Pueblo es una ficha más de la tabla B, sorteada sobre terreno abierto igual que Amenaza o Tesoro: no la trae maquetada ninguna loseta. Si sale «ninguna» a menudo, hay que subir su peso en TOKEN_WEIGHTS (lib/v2/rules/board-gen.ts).">
          <b>Pueblo:</b>{" "}
        ...
        <span title="Terreno transitable al que no se llega desde la entrada sin cruzar Montaña. La generación ya no abre la roca para arreglarlo: si aquí sale algo, la culpa es de una loseta cuya montaña parte su propio terreno, y se arregla en /lab/tiles.">
          <b>Incomunicado:</b>{" "}
          {stranded.length === 0
            ? "nada"
      ...
      {/* Lote de semillas: mismo generador y mismos mandos de arriba, mirando el
          reparto de N tableros en vez del ejemplar (lib/lab-registry.ts). */}
