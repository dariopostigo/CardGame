...
- **La dirección de arte de V3 está escrita**: `knowledge/v3/art-direction/style-guide.md` (la biblia visual, que gobierna todo el arte del juego y no cambió al cambiar el motor) e `knowledge/v3/art-direction/illustrations.md` (qué se dibuja: razas, héroes de clase, unidades y criaturas, con sus encuadres y plantilla de prompt). Esa carpeta cubre **solo el concepto de arte**: las medidas del archivo viven en `public/assets/v3/README.md`, y los sujetos con sus prompts ya montados en `knowledge/v3/races-concept/` (`sujetos.md` y `prompts/`). Con `razas.md` cerrado **ya se pueden ilustrar héroes y unidades** —empezando por Humanos, la raza piloto, que tiene sus 12 prompts escritos—; las ilustraciones de carta no, porque dependen del catálogo. Cuatro cosas colgando: el **lienzo** (1536×1050 es la medida heredada de v2, por confirmar contra el marco que construya V3), el **retrato**, que no tiene medida porque no hay pantalla de héroe ni ficha de unidad, el **concepto de calibración**, que no existe y lo será el primer héroe de Humanos aprobado, y las **cartas de clase**, que al cubrir todas las razas no pueden protagonizarlas ninguna.
- **El diseño de la carta como objeto —marco, tipografía, disposición del nombre y el coste, tratamiento de la Rareza— no está definido**, pero ya está **en marcha**: los conceptos de referencia se recogen y se analizan en `knowledge/v3/card-concept/`, con las imágenes guardadas ahí mismo, junto al texto. No es lo mismo que la ilustración que va dentro, y no lo cubre ningún documento de `knowledge/v3/art-direction/`. **Aviso que sale de ahí**: los tres conceptos valorados son **verticales y con el arte a sangre**, así que el lienzo apaisado de 1536×1050 heredado de v2 probablemente no sirve — se confirma cuando se cierre el marco.
- **Y ya se puede mirar**: los tres conceptos están dibujados como bocetos jugables en **[Cartas › Diseño de cartas](/docs/v3/cards/design)** (`components/design/v3/`, con sus estilos en `styles/components/card-sketch/`). No leen catálogo —no lo hay— sino seis sujetos de muestra escritos a mano que cubren los extremos: del 🗡️ Miliciano sin Características al 🐉 Dragón esquelético con cinco y tres cifras de Vida. Es la página que hay que mirar para cerrar el marco, y cuando uno gane se queda con él y absorbe a la de v2.
- **Y la lista ya crece por mezcla**: al boceto **D · Blindada** no lo respalda ninguna referencia nueva, sino el cruce del boceto C con el tema `armored` de v2 —el único marco de verdad que hay escrito—. Trae dos respuestas a decisiones abiertas: la **Rareza** deja de ser un filete y pasa a ser la aleación del marco entero, y el **Tier** sube a un medallón montado en la placa, con lo que el subtítulo se queda solo con la raza. Su coste está medido: la banda de metal se come 15px por lado y deja las cápsulas de la fila de ocho en ~30px, que es el techo para una Vida de tres cifras. Analizado en `knowledge/v3/card-concept/README.md` §"Mezclas".
- **Las Características llegan a CINCO, no a cuatro.** `knowledge/v3/card-concept/README.md` calculaba el marco para "de 0 a 4", pero en `razas.md` hay seis unidades de tier 8 con cinco (Dragón esquelético, Balor, Dragón ancestral, Kraken ancestral, Coloso mecánico y Abominación de plaga). Los bocetos ya están montados para cinco; lo que queda por decidir es si el tope se congela ahí o si alguna raza puede pasarlo.
