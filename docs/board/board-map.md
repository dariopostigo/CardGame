...
| Pueblo | — | — | — | — | — | **tope por tamaño, sin repetir** |

Con estos pesos, un mapa 12×12 lleva **~23 fichas** repartidas así: ~4,5 Enemigo · ~6,7 Amenaza · ~3,5 Tesoro · ~5,3 Personaje · ~2,4 Exploración · **~1,8 Terreno** (subido desde peso Bosque 1/Pantano 2, que daba ~1,2 — §4b). Es el reparto sobre el que están calibradas la **tabla de loot** y la **duración de 40 turnos** (`../game-design.md` §6b.6, §6c.1), así que tocar esta tabla obliga a revisar los dos; la subida de Terreno le quita una fracción pequeña a Amenaza y Enemigo en Bosque y Pantano, que son las de más peso en esos dos terrenos. Estimado a mano, pendiente de remedir con el lote de semillas (`../status.md` §6), igual que el resto de esta tabla. Medido sobre 300 tableros de losetas, la ficha de Personaje sale **5,53 veces por partida** (2,41 de Pueblo, 3,12 suelta en el resto del mapa).

**El Pueblo no se sortea con la tabla B, pero tampoco se llena entero *(decidido)*.** Es la única fila de la tabla que no es una tabla, y a propósito — un pueblo no es un sitio donde *puede* haber algo, es donde vive la gente, pero llenar TODOS sus hexágonos de golpe (un Poblado grande, cuatro NPC de una tacada) era lo que hacía que el mapa se sintiera con demasiada gente. Cada instancia de Pueblo da un **tope de NPC según cuántos hexágonos de Pueblo dibuja** — 1 ó 2 hexágonos dan uno, 4 dan dos (redondeo hacia arriba de la mitad, mínimo 1) — y **nunca repite oficio dentro de la misma instancia**: no puede haber dos taberneros en el mismo Poblado. El resto de hexágonos de Pueblo de esa instancia se queda sin ficha, como Pueblo vacío pero transitable. Las losetas de un solo hexágono no sortean: fijan su oficio por la nota de la loseta —la **Posada** es tabernero y se acabó, la **Iglesia** el sacerdote, la **Torre de mago** el mago—; solo **Poblado pequeño** y **Poblado grande** sortean entre el resto del catálogo (`../characters/npcs.md` §2), sin repetir. A diferencia de lo que decía antes este documento, *quién* es cada NPC ya no espera a la interacción: se decide aquí, en generación (`board-gen.ts`, `seedVillageNpcs`), precisamente para poder garantizar que no se repita oficio.
...
>
> **El Pueblo es además la puerta a su propia pantalla (E3)** *(añadido 2026-08-06)*: interactuar con cualquiera de sus NPCs abre la pantalla de Plaza + panel de oficio en vez de resolverse in-line sobre el hexágono — ver [`../characters/npcs.md`](../characters/npcs.md) §3c para el modelo completo (dos pantallas, stock independiente por jugador, coste de visita). El Pueblo **sigue siendo terreno multi-hexágono** como describe esta sección; solo cambia cómo se resuelve la interacción con la gente que vive ahí, no cuántos hexágonos ocupa ni qué tipo de loseta lo trae.
>
