...
>
> Dos consecuencias, y las dos están **sin remedir**: el reloj de Amenaza —**40 turnos**— está calibrado contra una travesía de **22 hexágonos** (`../game-design.md` §6c.1), y un tablero de 18 losetas es más largo que eso, así que hay que medir la travesía máxima de los tres tamaños antes de dar el reloj por bueno. La segunda es buena: más losetas, más probable que el encaje saque Mazmorra (§3b).
>
...
>
> El motivo es el mismo en los dos casos: ninguno era el arreglo del problema, sino su tapadera. La bolsa incomunicada viene de una loseta cuya roca parte su propio terreno, y abrirla dejaba un hexágono de Llanura en medio de la sierra que no correspondía a ninguna pieza. **Un tablero que se corrige a sí mismo esconde el problema en vez de enseñarlo**, y el problema siempre está en la biblioteca: en un dibujo o en un peso. Así que ahora los dos se **miden** y se informan (`GeneratedBoard.stranded`, que `/dev/tablero` enseña como «Incomunicado»; con la biblioteca de hoy, 0 en 300 tableros).
...
**Dónde se cumple la tabla A y dónde no.** El peso de cada tipo de loseta se ajusta para que **la bolsa** dé en la cuota. *(Cifras pendientes de remedir: al quitar los cinco tipos de loseta de Pueblo de la biblioteca cambió la composición entera de la bolsa, así que las medidas de "sobre 300 partidas" que había aquí antes ya no son válidas — hace falta un lote de semillas nuevo, `../status.md` §6.)*
...
Con estos pesos, un mapa 12×12 lleva **~23 fichas** repartidas entre las siete, con Terreno (~1,8/partida) y Pueblo en la franja más rara — Pueblo hereda en el Camino el papel que tenía la vieja Posada, "que un camino largo no sea un pasillo vacío". Es el reparto sobre el que están calibradas la **tabla de loot** y la **duración de 40 turnos** (`../game-design.md` §6b.6, §6c.1), así que tocar esta tabla obliga a revisar los dos. Primer pase sin balancear, pendiente de remedir con el lote de semillas (`../status.md` §6) — las cifras de "sobre 300 tableros" que había aquí antes quedaron obsoletas al quitar los tipos de loseta de Pueblo de la biblioteca, así que hace falta un lote nuevo.
...
> **Ojo, aparece poco (a propósito, pero un poco menos que antes).** Con la tabla B original (Pantano 2, Bosque 1) salía **~1,2 veces por mapa**. Se sube a **Pantano 3, Bosque 2** para llegar a **~1,8 por partida** — sigue siendo, con diferencia, la ficha más rara de las 7 —junto a Pueblo, que volvió a ser ficha con un peso igual de bajo— (es sabor de exploración, no un sistema central, así que **es la última que hay que implementar** del generador), pero ahora que también reparte carta merece aparecer un poco más. La subida le quita un pellizco pequeño al resto de fichas de Bosque y Pantano (sobre todo Amenaza y Enemigo, que son las de más peso ahí); las cifras exactas hay que remedirlas con el lote de semillas de `/dev/tablero` (`../status.md` §6), igual que el resto de la tabla.
