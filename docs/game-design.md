...
- **Base bajada de 3 a 2 (2026-08-05):** con `/dev/movement` jugado de verdad, los radios de `3 + mod SAB` se sentían demasiado largos frente al mapa real y al pool de 2 puntos de movimiento por turno. Un punto menos de base en los dos radios.
...
- [x] Definir el **rango de visión** en condiciones → §2.3: dos radios (detalle `2 + mod SAB` / terreno `detalle + 2`), escala +1 por punto de mod, con los invariantes `detalle > detección enemiga` y `terreno > detalle`. Cifras afinadas una vez jugado `/dev/movement` (2026-08-05); base bajada de 3 a 2.
