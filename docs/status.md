...
**Construido hasta hoy:** el tablero. Hexágonos y coordenadas, azar con semilla, los terrenos, el sistema de **losetas** con su biblioteca editable (17 tipos y 39 variantes tras retirar los 5 de Pueblo, los cinco tamaños) y la **generación del tablero** completa: encaje por anclas en los tres tamaños (**12, 15 ó 18 losetas**, 12 el mínimo), entrada, conectividad medida, Guarida garantizada, siembra de fichas —Pueblo incluido, otra vez ficha y no terreno— y huecos cerrados. Y el **diseño de las fichas** (`board/board-map.md` §4c): las 7 de contenido y las 3 de personaje, todas en el mismo disco tumbado y con sus cuatro estados. Se trabaja en `/dev/tiles`, `/dev/board` y `/dev/pieces`.

**Lo inmediato, dentro del tablero:**
- ~~El lote de semillas~~ **construido y corrido** *(2026-08-10)*: `/dev/board` genera cientos de tableros de golpe y mide el reparto en vez del ejemplar. Ya remidió las cifras de 9 losetas / biblioteca vieja que citaban este documento y `board/board-map.md` — los números de abajo y los de `board/board-map.md` §2c son los nuevos, sacados de N=300 por tamaño (12/15/18 losetas).
- **Aparcado a propósito, no olvidado** *(decidido 2026-08-10)*: el reloj de Amenaza —**40 turnos**— está calibrado contra una travesía de 22 hexágonos (`game-design.md` §6c.1), y el lote mide una travesía máxima media de **24,2 hexágonos ya en el tablero mínimo** (12 losetas), subiendo a **30,8** con 18 (`board/board-map.md` §2c). Pero esa cifra es distancia en hexágonos sin coste de movimiento — el bucle de turno/movimiento (punto 2 de "Orden de lo que queda", más abajo) todavía no existe, así que cuántos turnos hacen falta de verdad sigue siendo una suposición. Se decide con datos reales en cuanto exista ese bucle, no antes.
- ~~Peso de la ficha de **Terreno**~~ **subido y remedido** *(aplicado 2026-08-10)*: `TOKEN_WEIGHTS` (`lib/rules/board-gen.ts`) pasó de Pantano 2/Bosque 1 a **Pantano 3/Bosque 2**, la subida que ya estaba escrita en `board/board-map.md` §4b. Sube de 0,86 a **1,27/partida** con 12 losetas y **1,81** con 18 — el ~1,8 objetivo resultó ser la cifra del tablero grande, no del mínimo.
- **Decisión abierta:** Camino y Montaña salen en el tablero por debajo de su cuota de bolsa (16,2 % vs 20 % · 9,1 % vs 10 %) porque tienen anclas restringidas. Subir pesos o dar preferencia al ancla de camino al colocar (`board/board-map.md` §2c).
...
1. **Fichas** en el tablero por orden de frecuencia (tablero de 12 losetas, lote de semillas): Amenaza (~3,8/mapa) → Enemigo ≈ Personaje (~2,7) → Tesoro (~2,0) → Pueblo (~1,7) → Exploración (~1,5) → **Terreno al final** (~1,3 por mapa, `board/board-map.md` §4b).
