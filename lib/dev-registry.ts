...
export const DEV_MODULES: readonly DevModule[] = [
    ...
    needs: [
      {
        ...
        standIn:
          "las cinco Habilidades de escalón (🛡️ 🔮 🎯 🍀 ⚡) en su valor por defecto, iguales en las 24 fichas, porque razas.md todavía no dice cuál le toca a cada una — solo ❤️ Vida y ⚔️ Ataque son propias de la ficha. Lo que SÍ dejó de ser sustituto el 11 de septiembre de 2026 son los sujetos: la pantalla comía de los 48 escritos a mano de components/design/v3/ —con sus propias 8 Habilidades y sus números inventados— y ahora lee el mismo roster que reparte /dev/baraja, así que el ⛏️ Minero es uno solo",
      },
  ...
  },
  {
    ...
    needs: [
      { slug: "cartas", what: "qué se baraja", standIn: "las 16 cartas de UNIDAD de Humanos y Enanos, repetidas cíclicamente — Dario, 9-sep-2026: «me da igual que sea real, quiero ver el sistema en sí»" },
      {
        ...
        standIn:
          "un retal de quince hexágonos, que YA NO ESTÁ COPIADO: desde el 11-sep-2026 la geometría es lib/v3/patch.ts (`fitPatch`), el suelo que se pinta es components/dev/HexPatch.tsx con su parcial _hex-patch.scss, y las secuencias son components/dev/motion/ — los mismos archivos que usa /dev/animacion, así que los dos retales son literalmente el mismo. Antes eran dos copias con dos márgenes distintos (96 y 48 px) y dos prefijos de clase, y el aliento —que también estaba dos veces— ya había divergido cuando se fue a mirar: aquí animaba el grupo SVG y allí el disco con su centrado. Lo que sigue siendo sustituto es que esto sea un RETAL y no la arena: quince hexágonos quietos, sin cámara, sin bandas y sin alcances. Fundirlo con ArenaBoard es lo que queda, y es trabajo del módulo «tablero» — Dario ya lo nombró el 9-sep-2026: «después lógicamente los dos UNIFICADOS en el contenido del tablero»",
      },
  ...
  // --------------------------------------------------------------- sensación
  {
    ...
    needs: [
      {
        ...
        standIn:
          "un retal de quince hexágonos con la geometría y la cámara de la arena, pero sin ser la arena: sin bandas, sin alcances y sin cámara arrastrable. Ya no está copiado a mano —lib/v3/patch.ts y components/dev/HexPatch.tsx desde el 11-sep-2026, compartidos con /dev/baraja—, y lo que falta por hacer es fundirlo con ArenaBoard",
      },
      {
        slug: "pieza",
    ...
    blocker:
      "EL CATÁLOGO DE ANIMACIONES, 11-sep-2026: la pantalla ya no es un banco y una caja de cuarenta diales. Dario lo pidió al ver que esta pantalla y /dev/baraja se parecían cada vez más: «¿cómo ves unificarlas y separar más atómicamente lo que es animación en sí? puede salir un pequeño preview para cada una de las animaciones más aisladas para poder modificar sus propiedades». Se hizo en dos mitades. La de código: cada secuencia es ahora un archivo de components/dev/motion/ —despliegue, oferta, paso, aliento, embestida con sus tres desenlaces, muerte— con el mismo contrato que ya tenía el despliegue (nodos y puntos ya medidos, cero React, cero saber qué es una ficha); vivían dentro de AnimationBench.tsx, unas 560 líneas, y mientras vivieran ahí no había forma de mirar UNA sin montar la pantalla entera. La de datos: `ANIMATIONS` (lib/v3/anim.ts) dice qué animaciones existen, qué pregunta contesta cada una, qué escena necesita y qué diales la mueven — `KNOBS.group` no servía, porque agrupa por familia y «impacto» mete en la misma caja la embestida, el contacto y la muerte. De ahí salen los nueve bancos pequeños del catálogo. DOS REGLAS QUE NO SON DE ESTILO: (1) UN DIAL, UN VALOR — los previews no tienen estado propio de tiempos, leen y escriben el mismo `Timings` que el banco grande, o volverían las «dos respuestas a la misma pregunta» que este módulo acaba de pagar dos veces; comprobado en Chrome que mover un slider en cualquiera de los dos sitios mueve el otro. (2) EL PREVIEW NO SUSTITUYE AL BANCO, y por eso el banco sigue arriba y entero: aislada, cada secuencia parece bien, y lo que no se ve de una en una es la MEZCLA — que lo gastado se lee por AUSENCIA del aliento (y una ausencia solo se ve si lo demás está presente) y que el ritmo no se juzga mirando un ataque, que es para lo que existe la tanda de doce. Corolario práctico: nada se reproduce en bucle, porque `idleRise` viene a 0 justamente para que un bucle infinito no se cuele por debajo de lo que estés juzgando, y nueve previews respirando a la vez serían ese problema por nueve. Para seguir midiendo no hay bloqueo: los cuatro sustitutos son deliberados, porque una caída no necesita saber cuánto pega la ficha, y mezclar el movimiento con el aspecto haría que se juzgaran los dos a la vez y mal. Lo que sí espera es al motor de combate, y al revés de lo normal: no lo necesita para funcionar, es el motor el que va a tener que nacer emitiendo SUCESOS en vez de mutando estado, o nada de esto se podrá enseñar (lib/v3/anim.ts, `schedule`). Y EL 7 DE SEPTIEMBRE DE 2026 SE ESCRIBIÓ EL PUENTE. Dos huecos, y los dos cerrados: `schedule()` ya acepta lotes (`Batch`) además de sucesos sueltos, así que un grupo de sucesos simultáneos —el tic de estados al empezar el turno— se escalona con `stagger` en vez de ponerse en fila, y lo siguiente espera a que termine el último del lote, no el primero. Y lib/v3/battle.ts `toAnimEvents()` traduce lo que jugó `fight()` —paso, ataque, muerte— al vocabulario de aquí, así que un combate real ya se puede convertir en una cola de animación entera (probado con node: un 1 contra 1 completo sale en 11 sucesos y 5,5 segundos de cola). Lo que sigue faltando no es de lib/v3/: es reproducir esa cola contra el estado de una partida en esta pantalla —hoy el puente solo está probado fuera de React— y que los nueve estados existan para tener con qué llenar un lote de verdad. La oferta y el andar ya no piden nada: las cuentas las hace lib/v3/movement.ts, que existía desde el duelo del tablero, así que el banco pregunta y no calcula. Lo que sí queda abierto es si la onda de la oferta entra en presupuesto sobre el 14×12 de verdad —en el retal de quince siempre cabe—, y eso solo se ve mudando esto a la arena.",
  },
];
