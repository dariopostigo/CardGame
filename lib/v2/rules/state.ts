...
// leído de docs/cards/*.md) y lib/v2/rules/deck.ts (Mazo/Oteo). `Character`,
// `Enemy` y `Combat` pasan de "pendiente" a tipos base (sin motor de combate
// todavía — eso es lib/v2/rules/combat.ts, que se construye subsistema a
// subsistema).
//
...
//     hexágonos. Se maqueta a mano en la biblioteca (lib/v2/rules/tile-library.ts)
//     y se edita en /lab/tiles. Las variantes de un mismo TIPO son el mismo
//     sitio dibujado de otra manera (`TileType` en lib/v2/rules/tiles.ts).
...
//     board-gen.ts y se prueba en /lab/board.
// =========================================================================
...
export type PlacedTile = {
  ...
  readonly id: string;
  /** Loseta de la biblioteca de la que sale (lib/v2/rules/tile-library.ts). */
