...
**Cada documento de V3 declara su estado, y lo declara en sí mismo** *(5 de septiembre de 2026)*. Una línea `<!-- estado: por-escribir | a-medias | escrito | en-espera -->` arriba del `.md`, que lee [`lib/docs.ts`](lib/docs.ts) y pinta el menú, para contestar desde la navegación si algo hay que definirlo, está a medias, ya está o está parado. Va en el documento y no en una tabla aparte porque es lo único que no puede desincronizarse de lo que describe; el precedente es `<!-- cards: … -->` ([`lib/card-table.ts`](lib/card-table.ts)), y `react-markdown` no renderiza HTML crudo, así que la línea no se ve. Un valor mal escrito **revienta** en vez de pasar en silencio: un estado que miente es peor que ninguno. Tres reglas al tocarlo, en [`lib/doc-status.ts`](lib/doc-status.ts):
