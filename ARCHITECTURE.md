...
| **tipo de loseta** | Un **sitio** del mundo (un peñasco, una ciénaga, una posada), definido por **un terreno**, con su peso en la bolsa y sus variantes dentro | `TileType` ([`lib/rules/tiles.ts`](lib/rules/tiles.ts)) | `/dev/losetas` |
...
Y al revés: **la generación no repinta NADA.** `board-gen.ts` no sortea terreno y tampoco lo corrige: cada hexágono del tablero es el que dibujó su loseta, sin excepciones. Tuvo tres y se le han quitado las tres, porque un tablero que se corrige a sí mismo esconde el problema en vez de enseñarlo: ya no abre la Montaña que encierra una bolsa (la **cuenta** en `GeneratedBoard.stranded`, que `/dev/tablero` enseña como «Incomunicado»), ya no mueve la entrada que cae en roca (`pickEntrance` elige por terreno y no le hace falta) y ya no **funda un Pueblo** cuando el encaje no saca ninguna loseta de Pueblo. Las tres eran síntomas de otra cosa —un maquetado que parte su propio terreno, una bolsa con pocos pueblos— y las tres se arreglan donde se decide: en la loseta o en el peso del tipo. `typeNotes` avisa de la primera al maquetar; de la segunda avisa el propio contador del laboratorio saliendo a «Pueblo: ninguno».
...
| `board-gen.ts` | Generación del **tablero**: unión de **12, 15 ó 18** losetas por sus anclas + entrada por la boca del camino + Guarida del boss + fichas. No repinta terreno: la conectividad se **mide** (`stranded`) y el Pueblo sale si el encaje lo trae |
