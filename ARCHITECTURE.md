...
- **El arte sí está partido**, igual que el diseño: [`public/assets/v2/`](public/assets/v2/cards/README.md) (congelado) y [`public/assets/v3/`](public/assets/v3/README.md) (vacío, para lo nuevo). Se sirven en `/assets/v2/…` y `/assets/v3/…`; el corte está dentro de `assets/` y no en la raíz de `public/` para que una carpeta de arte no se confunda con una ruta de la app. Los `.glb` prestados de laboratorio están **dentro** del corte, en [`public/assets/v2/models/`](public/assets/v2/models/README.md): no son arte del juego, pero los cargan los laboratorios de v2 y se van con ellos. Queda **fuera** [`public/concepts/`](public/concepts/README.md) (moodboards), que no se sirve al jugador y no es de ninguna versión. Si algo de `v2/` sirve para V3 **se copia**, no se enlaza: `v2/` debe poder vaciarse sin romper nada de V3.
...
| **Dev** | [`app/dev/`](app/dev/) | la **construcción de V3**: un módulo por sistema, **en cadena de dependencia** ([`lib/dev-registry.ts`](lib/dev-registry.ts)) | herramienta |
...
> `lib/docs-version.ts` existe separado de [`lib/docs.ts`](lib/docs.ts) por una razón concreta: `docs.ts` usa `node:fs` y es solo-servidor, pero la cabecera, las migas y el buscador son componentes cliente que necesitan saber en qué versión están. Importarlos desde `docs.ts` mete `fs` en el bundle del navegador y el build de Turbopack falla. Por lo mismo está aparte [`lib/doc-status.ts`](lib/doc-status.ts), que lo importa el menú.

**El árbol de V3 dejó de ser el de v2** *(6 de septiembre de 2026)*. Los grupos del menú se declaran una vez en `GROUP_SHAPE` ([`lib/docs.ts`](lib/docs.ts)) y se generan para las dos versiones; los que se quedan sin documentos se descartan solos, y por eso v2 no enseña los dos nuevos. Eran cuatro —General, Tablero, Personajes, Cartas—, las carpetas de v2, y V3 las heredó por una decisión explícita de la migración: *«`docs/v3/` es espejo estructural de v2, con contenido nuevo»* ([`knowledge/v3/migracion-v2-v3.md`](knowledge/v3/migracion-v2-v3.md)), para que la estructura existiera antes que el contenido. Al medirlo dos semanas después: **17 de los 18 documentos de V3 tenían contraparte en v2 con el mismo nombre y el mismo sitio**, y el único nativo —`razas.md`— se había comido el **43% del texto del árbol** con cinco temas dentro. Se parte en dos grupos nuevos:

- **`sistemas/`** es lo que llevan las **132 fichas por igual** y no es de nadie en particular: [`ficha.md`](docs/v3/sistemas/ficha.md) (qué campos existen), `habilidades.md`, `caracteristicas.md`, `dano.md` y `effects.md`, que se muda aquí desde la raíz. Estaba escondido dentro de las razas, y se notaba: cuatro filas del glosario mandaban a `razas.md` a buscar la definición de *Habilidad*, *Característica* y *Tipo de daño*.
- **`razas/`** pasa de archivo a grupo con lo que sí es suyo: las 11 razas con sus clases (el índice) y sus 88 unidades. **El índice conserva el slug `/docs/v3/razas`** —`README.md` es el índice de su carpeta—, así que las rutas cableadas de [`lib/dev-registry.ts`](lib/dev-registry.ts) y de los labs siguen valiendo sin tocarlas.

El criterio para lo que venga: **si vale para las 132 fichas, es Sistemas; si es de una raza, es Razas; si es lo que imprime una carta, es Cartas.** Una unidad vive en los tres sitios y no es contradicción — su ficha en `razas/unidades.md`, sus campos en `sistemas/ficha.md`, su carta en `cards/units.md`.

> Los nombres nuevos van en **español** y los heredados de la estructura de v2 conservan el suyo (`effects.md`, `game-design.md`, `board/battle.md`…). No es un descuido: renombrarlos rompe rutas leídas en runtime —`lib/v3/effects-catalog.ts` abre el `.md` por ruta— a cambio de nada.

**Cada documento de V3 declara su estado, y lo declara en sí mismo** *(5 de septiembre de 2026)*. Una línea `<!-- estado: por-escribir | a-medias | escrito | en-espera -->` arriba del `.md`, que lee [`lib/docs.ts`](lib/docs.ts) y pinta el menú, para contestar desde la navegación si algo hay que definirlo, está a medias, ya está o está parado. Va en el documento y no en una tabla aparte porque es lo único que no puede desincronizarse de lo que describe; el precedente es `<!-- cards: … -->` ([`lib/card-table.ts`](lib/card-table.ts)). **La línea se borra antes de pintar** (`preprocess()` en `lib/docs.ts`): sin `rehype-raw`, react-markdown no interpreta el comentario HTML pero lo escapa y lo enseña como texto encima del título — se dio por hecho lo contrario y estuvo un día a la vista. Un valor mal escrito **revienta** en vez de pasar en silencio: un estado que miente es peor que ninguno. Tres reglas al tocarlo, en [`lib/doc-status.ts`](lib/doc-status.ts):

- **No se etiqueta todo, solo lo que se sale de la norma.** El grupo dice su estado por defecto una vez, en la cabecera, y las entradas que lo comparten se callan. Se midió el día de escribirlo: **14 de los 18 documentos de V3 empezaban por "Esqueleto"**, así que etiquetarlas todas era una columna de palabras repetidas que tapaba el caso raro. La norma exige **mayoría de todas las entradas del grupo**, no solo de las que declaran: si no, un grupo mezclado como *General* le colgaría un estado al Glosario que este nunca dijo.
- **Los índices y los documentos vivos no declaran nada** —Estado, Ideas, Glosario, los README—. No son diseño, son andamios: crecen siempre, así que cualquiera de las cuatro palabras les mentiría y no se les quitaría nunca.
- **`en-espera` no marca, apaga.** La entrada deja de ser enlace y va tachada, porque no espera a que alguien la escriba sino a que se decida si sigue existiendo (hoy solo [`docs/v3/cards/curses.md`](docs/v3/cards/curses.md)). Nunca puede ser la norma de un grupo, o dejaría de verse justo lo que se quería ver. Se llega al documento desde el índice de su grupo, que es donde está contado por qué está parado.

v2 no declara estado en ningún archivo: está congelada entera y eso ya se dice una vez, en su entrada de índice. Y esto **no es el `BuildStatus` de [`lib/sections.ts`](lib/sections.ts)** (`listo`/`en curso`/`planificado`) que gastan `/dev` y los repositorios: aquel mide si algo está **construido**, y un documento de diseño puede estar escrito entero sobre algo sin decidir —`ideas.md` es exactamente eso—.
...
**Y ya divergen los registros, que es la diferencia de fondo entre las dos secciones** *(1 de septiembre de 2026)*. Los laboratorios de `/lab` son **independientes**: cada uno aísla una pieza del motor v2 y su orden en la lista es de uso, fijado a mano. Los módulos de `/dev` son una **cadena**: `DevModule.needs` declara de qué otro módulo necesita algo y **qué** le pide, así que el orden de la lista es topológico y `dependencyProblems()` lo comprueba —el hub pinta en rojo si alguien apunta a un slug que no existe o a uno que va después—. Tres consecuencias que no son de estilo:
...
│   └── v2/                 #   juego anterior (cards/, sprites/) + models/ (.glb prestados de lab), congelado
